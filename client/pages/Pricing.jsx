...
console.log(error)
    ...
    return (
      ...
      hover:text-white transition' onClick={() => navigate("/")}>
            ...
            <div className='relative z-10 max-w-7xl mx-auto gap-8 grid grid-cols-1 md:grid-cols-3 '>
                {plans.map((p, i) => (
                    <motion.div
                        key={i}
                        initial={{ opacity: 0, y: 40 }}
                        whileInView={{ opacity: 1, y: 0 }}
                        transiton={{ delay: i * 0.12 }}
                        whileHover={{ y: -14, scale: 1.03 }}
                        className={`relative rounded-3xl p-8 border backdrop-blur-xl ransition-all ${p.popular
